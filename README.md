...
- **`hosts/nixos-host.nix`**
  - NixOS configuration for the **NixOS-host VM** (libvirt/qemu, kubectl/talosctl tooling, container registry, and proxy services).
...
- **`k8s/apps/frontend-demo/`**
  ...
  - Exposed via Traefik Ingress at `https://demo.lab.local`.
...
- `/etc/talos-ingress-proxy.env` (generated by the ingress command)
...
All monitoring services are accessed via Traefik Ingress on HTTPS:

- **Grafana**: https://grafana.lab.local (admin/admin)
- **Prometheus**: https://prometheus.lab.local

See [section 11](#11-ingress-with-traefik-tls) for SSH tunnel setup to access from your laptop.
